...
export class CreateMovieDto implements ICreateMovieDto {
  @JoiSchema(Joi.string().max(255).required().messages({
  ...
  @JoiSchema(Joi.number().required().min(YEAR_OF_CREATION_OF_THE_FIRST_FILM).messages({
  ...
  @JoiSchema(Joi.number().min(Number.MIN_VALUE).required().messages({
  ...
  @JoiSchema(Joi.array().items(Joi.string()).required().messages({
  ...
  @JoiSchema(Joi.string().optional().messages({
  ...
  @JoiSchema(Joi.string().uri().optional().messages({
